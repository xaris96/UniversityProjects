...
RUNTIME_PROFILE = "speed"  # "speed" | "balanced" | "robust"
...
# Freeze a fast/high-performing setup from your latest run
...
if RUNTIME_PROFILE == "speed":
    TRIALS_RF = 0
    TRIALS_ET = 0
    TRIALS_XGB = 1 if USE_FIXED_XGB_PARAMS else 4
    TRIALS_MLP = 0
    TUNE_SPLITS = 2
    FINAL_SPLITS = 3
    PERM_REPEATS = 1
...
print(
    ...
    f"Models={MODELS_TO_TUNE} | Trials (RF/ET/XGB/MLP) = {TRIALS_RF}/{TRIALS_ET}/{TRIALS_XGB}/{TRIALS_MLP} | "
...
for name in MODELS_TO_TUNE:
    ...
    if name == "XGBoost" and USE_FIXED_XGB_PARAMS:
    ...
    if name == "XGBoost" and (not USE_FIXED_XGB_PARAMS):
        xgb_anchor_params = [
            {
                "n_estimators": 1600,
                "max_depth": 7,
...
# Feature importance plots (saved + displayed)
ARTIFACTS_DIR = Path("artifacts")
ARTIFACTS_DIR.mkdir(exist_ok=True)
...
model_fi_save = ARTIFACTS_DIR / f"feature_importance_{best_model.lower()}_model.png"
if hasattr(final_model, "feature_importances_"):
    scores = final_model.feature_importances_
    fi_model = plot_feature_importance(selected_features, scores, topk=15, title=f"{best_model} feature importance", save_path=model_fi_save)
elif hasattr(final_model, "coef_"):
    scores = np.abs(np.asarray(final_model.coef_).ravel())
    fi_model = plot_feature_importance(selected_features, scores, topk=15, title=f"{best_model} |coef|", save_path=model_fi_save)

if RUN_PERM_IMPORTANCE:
    perm_scores = permutation_importance_generic(
        final_model,
        X_model,
        y,
        selected_features,
    ...
    perm_fi_save = ARTIFACTS_DIR / f"feature_importance_{best_model.lower()}_permutation.png"
    fi_perm = plot_feature_importance(
        selected_features,
        perm_scores,
        topk=15,
        title=f"Permutation importance ({best_model})",
        save_path=perm_fi_save,
    )

# Optional SHAP (for XGBoost) with safe fallback
if ENABLE_SHAP and best_model == "XGBoost":
    try:
        import shap

        if len(X_model) > SHAP_MAX_SAMPLES:
            rng = np.random.RandomState(SEED)
            idx = rng.choice(len(X_model), size=SHAP_MAX_SAMPLES, replace=False)
            X_shap = X_model.iloc[idx]
        else:
            X_shap = X_model
        ...
        if shap_arr.ndim == 3:
            shap_arr = shap_arr[:, :, 0]
        shap_scores = np.mean(np.abs(shap_arr), axis=0)
        fi_shap = (
            pd.DataFrame({"Feature": selected_features, "Score": shap_scores})
            .sort_values("Score", ascending=False)
            .head(15)
            .reset_index(drop=True)
        )
        display(fi_shap)
        fi_shap.to_csv(ARTIFACTS_DIR / "feature_importance_shap_top15.csv", index=False)
    except Exception as e:
        print(f"SHAP skipped: {e}")

if fi_model is not None:
    display(fi_model)
    fi_model.to_csv(ARTIFACTS_DIR / "feature_importance_top15.csv", index=False)
if fi_perm is not None:
    display(fi_perm)
    fi_perm.to_csv(ARTIFACTS_DIR / "feature_importance_permutation_top15.csv", index=False)
...
ablation_df = None
if RUN_ABLATION and ("EV_raw_Diff" in selected_features) and (len(selected_features) > 1):
    ablation_features = [f for f in selected_features if f != "EV_raw_Diff"]
    X_ablation = X[ablation_features].copy()
    X_ablation_values = X_ablation.values

    if best_model == "XGBoost":
        ablation_oof = cv_oof_xgb_bagged(
        ...
        ablation_oof = cv_oof_sklearn(
            build_fn=cfg["build"],
            params=best_params[best_model]["params"],
            X=X_ablation,
            y=y,
            w=w_model,
            groups=groups,
            ...
            scale=cfg["scale"],
            splits=final_splits,
            X_values=X_ablation_values,
        )

    ablation_oof = clip01(ablation_oof)
    ab_mae, ab_rmse, ab_r2 = metrics(y, ablation_oof)
    ablation_df = pd.DataFrame([
        {
            "feature_removed": "EV_raw_Diff",
            "base_mae": float(mae),
            "ablation_mae": float(ab_mae),
            "mae_delta": float(ab_mae - mae),
            "base_rmse": float(rmse),
            "ablation_rmse": float(ab_rmse),
            "base_r2": float(r2),
            "ablation_r2": float(ab_r2),
        }
    ])
    display(ablation_df)
    ablation_df.to_csv(ARTIFACTS_DIR / "ablation_ev_raw_diff.csv", index=False)
...
applied_sweep_k = int(X_model.shape[1]) if "sweep_df" in globals() and RUN_SWEEP and APPLY_SWEEP_BEST_K else None
ablation_mae_delta = None if ablation_df is None else float(ablation_df.iloc[0]["mae_delta"])
...
report = {
    ...
    "applied_sweep_k": applied_sweep_k,
    "use_fixed_features": bool(USE_FIXED_FEATURES),
    "use_fixed_xgb_params": bool(USE_FIXED_XGB_PARAMS),
    "xgb_oof_seeds": [int(s) for s in XGB_OOF_SEEDS],
    "run_perm_importance": bool(RUN_PERM_IMPORTANCE),
    "run_ablation": bool(RUN_ABLATION),
    "ablation_mae_delta": ablation_mae_delta,
    "seed": int(SEED),
